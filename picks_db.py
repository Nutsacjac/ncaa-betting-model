"""
SQLite database layer for tracking NCAA betting picks and outcomes.
"""
...
def resolve_pick(pick_id, home_score, away_score):
    ...
    if market == "Spread":
        ...
        if spread is not None:
            covered_margin = actual_margin + spread  # home perspective + spread
...
def get_stats(market=None, status=None, date_from=None, date_to=None):
    ...
    # ROI calculation: flat 1u at -110 juice for spread/O/U, actual ML odds for ML
    profit = 0.0
    total_wagered = 0
    for p in resolved:
        if p["status"] == "push":
        ...
        total_wagered += 1
        if p["best_market"] == "ML":
            # Use actual moneyline odds for payout
            ml_odds = None
            ...
            # Spread and O/U at -110
            if p["status"] == "won":
                profit += 100 / 110  # ~0.909
            else:
                profit -= 1
