...
class ESPNDataProvider:
    ...
    def search_team(name):
        ...
        try:
            ...
            resp = requests.get(url, params={"limit": 400}, timeout=REQUEST_TIMEOUT)
            resp.raise_for_status()
            data = resp.json()
...
class TheOddsAPIProvider:
    ...
    def _parse_event(event):
        ...
        # Aggregate odds from the first available bookmaker for each market
        spread = over_under = home_ml = away_ml = None
        ...
        for bookmaker in event.get("bookmakers", []):
            ...
            # Stop after the first bookmaker that has data
